...
func TestFifo_Put(t *testing.T) {
	...
	tests := []struct {
		...
		{
			args: args{
				d: Cache.Data{Key: "4", Ele: "four"},
			},
		},
		{
			args: args{
				d: Cache.Data{Key: "3", Ele: "three"},
			},
		},
	}
	for _, tt := range tests {
...
func TestFifo_Get(t *testing.T) {
	c, _ := CreateFifoCache(3)
	f := c.(*Fifo)
	f.Put(Cache.Data{Key: "1", Ele: "one"})
	f.Put(Cache.Data{Key: "2", Ele: "two"})
	r := Redis.NewRedis()
	r.Put(Cache.Data{Key: "3", Ele: "three"})
	type args struct {
		key string
	}
	tests := []struct {
		...
		// TODO: Add test cases.
		{
			...
			wantData: Cache.Data{Key: "2", Ele: "two"},
			wantErr:  false,
		},
		{
			name: "Test2",
			...
			wantData: Cache.Data{Key: "3", Ele: "three"},
			wantErr:  false,
		},
		{
			name: "Test3",
